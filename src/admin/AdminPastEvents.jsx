...
const AdminPastEvents = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingEvent) {
        ...
        await firestoreService.add(COLLECTIONS.PAST_EVENTS, formData);
      }
      setIsFormOpen(false);
    } catch (error) {
  ...
  const confirmDelete = async () => {
    try {
      await firestoreService.delete(COLLECTIONS.PAST_EVENTS, deletingEvent.id);
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-3xl font-bold text-gray-800">Manage Past Events</h2>
        <button
          onClick={handleAdd}
      ...
      {events.length === 0 && (
        <div className="text-center py-12">
          <p className="text-gray-500 text-lg">No past events yet. Add your first event!</p>
        </div>
      )}
